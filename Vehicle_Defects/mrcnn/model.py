...
import sys
#from mrcnn import parallel_model as ParallelModel
...
class BatchNorm(KL.BatchNormalization):
    ...
    def call(self, inputs, training=None):
        ...
        """
        return super(self.__class__, self).call(inputs, training=training) #original training=training
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            keras.callbacks.EarlyStopping(patience=3, monitor="val_loss", mode="min",
                                            verbose=1),
        ]
...
class MeanAveragePrecisionCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if (epoch + 1) % self.calculate_map_at_every_X_epoch == 0:
            ...
            mAPs, Scores = self._calculate_mean_average_precision()
            mAP = np.mean(mAPs)
            Score = np.mean(Scores)

            if logs is not None:
                logs["val_mean_average_precision"] = mAP
                logs["val_mean_average_accuracy"] = Score

            self._verbose_print("mAP at epoch {0} is (%) : {1:.2f}".format(epoch+1, mAP))
            self._verbose_print("Average accuracy is (%) : {0:.2f}".format(Score))

        super().on_epoch_end(epoch, logs)

    def _load_weights_for_model(self):
        last_weights_path = self.train_model.find_last()
        self._verbose_print("Loaded weights for the inference model (last checkpoint of the train model): {0}".format(last_weights_path))
    ...
    def _calculate_mean_average_precision(self):
        ...
        for index, image_id in enumerate(self.dataset_image_ids[:self.dataset_limit]):
            image, image_meta, gt_class_id, gt_bbox, gt_mask = load_image_gt(self.dataset, self.inference_model.config, image_id)
            ...
            score = (2 * (np.mean(precisions) * np.mean(recalls))) / (np.mean(precisions) + np.mean(recalls))
            if score > 0.5:
                success += 1
                Scores.append(score)
                mAPs.append(AP)
            ...
            if index != 0: print("\r" + str(index + 1) + " images done     ", end = "")
        self._verbose_print("\nDetected = {0}, No detections = {1}".format(success, failed))
        return np.array(mAPs), np.array(Scores)

def EvalImage(dataset, infer_model, train_model=None):
    APs, F1_Scores = [], []
    success, failed = 0, 0
    if train_model is not None:
        last_weights_path = train_model.find_last()
        print("Loaded weights for the inference model (last checkpoint of the train model): {0}".format(last_weights_path))
        infer_model.load_weights(last_weights_path, by_name=True)

    for index, image_id in enumerate(dataset.image_ids):
        # if(index > len): break
        
        # load image, bounding boxes and masks for the image id
        image, image_meta, gt_class_id, gt_bbox, gt_mask = load_image_gt(dataset, infer_model.config, image_id)
        # convert pixel values (e.g. center)
        #scaled_image = modellib.mold_image(image, self.cfg)
        # convert image into one sample
        molded_images = np.expand_dims(mold_image(image, infer_model.config), 0)
        results = infer_model.detect(molded_images, verbose=0)
        r = results[0]
        ...
        if index != 0: print("\r" + str(index + 1) + " images done     ", end = "")
    print("\nDetected = {0}, No detections = {1}".format(success, failed))
    return (np.mean(np.array(APs)) * 100), (np.mean(np.array(F1_Scores)) * 100)